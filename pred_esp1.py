...
df = pd.read_excel("Exp_narciso_nanom3.xlsx")
df = df.drop_duplicates(subset=['E'])

# Interpolación a 50 puntos
energias_interp = np.linspace(df['E'].min(), df['E'].max(), 100)
interp_psi = interp1d(df['E'], df['psi'], kind='linear', fill_value="extrapolate")
interp_delta = interp1d(df['E'], df['delta'], kind='linear', fill_value="extrapolate")
...
df_fit2=calcular_psi_delta_semi(188.5554,  2.0330320 , 2.8751030, 2.0907410, 1.4453730, 190.7478, 70, E_values=energias_interp)
...
# axs[0, 0]: Ψ NN
axs[0, 0].plot(df['E'], df['psi'], 'o', label=r'$\Psi$ Expl')
...
# axs[0, 1]: Δ NN
axs[0, 1].plot(df['E'], df['delta'], 'o', label=r'$\Delta$ Exp')
...
# axs[1, 0]: Ψ PINN
axs[1, 0].plot(df['E'], df['psi'], 'o', label=r'$\Psi$ Exp')
...
# axs[1, 1]: Δ PINN
axs[1, 1].plot(df['E'], df['delta'], 'o', label=r'$\Delta$ Exp')
...
psi_exp_interp = interp1d(df['E'], df['psi'], kind='linear', fill_value="extrapolate")(energias_interp)
delta_exp_interp = interp1d(df['E'], df['delta'], kind='linear', fill_value="extrapolate")(energias_interp)
...
eps1_fit = tauc_lorentz_eps1(energias_interp, 188.5554, 2.0330320, 2.8751030, 2.0907410, 1.4453730)
eps2_fit = tauc_lorentz_eps2(energias_interp, 188.5554, 2.0330320, 2.8751030, 2.0907410)
