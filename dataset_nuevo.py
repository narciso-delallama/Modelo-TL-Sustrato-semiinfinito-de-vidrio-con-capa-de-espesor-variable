...
for spectrum_id in range(n_spectra):
    ...
    # Calcular espectro
    df_temp = calcular_psi_delta_semi(A, E0, C, Eg, eps_inf, d_film, 70, np.linspace(0.5, 6.5, 50))
    df_temp['A'] = A
    df_temp['E0'] = E0
    df_temp['C'] = C
    df_temp['Eg'] = Eg
...
# Reordenar columnas
column_order = ['A', 'C', 'E0', 'Eg', 'eps_inf', 'd_film', 'E', 'psi (deg)', 'delta (deg)']
df_final = df_final[column_order]

# Guardar a CSV (opcional)
df_final.to_csv("dataset_nuevo_semi.txt", sep='\t', index=False)
print("Dataset generado y guardado como 'dataset_nuevo_semi.txt'")
